...
function EventRow({ event }: { event: ActivityEvent }) {
  ...
  return (
    <div className={cn('flex items-start gap-3 py-2 first:pt-0', muted && 'opacity-60')}>
      ...
      <p className="flex-shrink-0 text-xs text-muted-foreground tabular-nums">{formatTime(event.timestamp)}</p>
    </div>
  )
}
...
export function ActivityFeed({ events, isLoading }: ActivityFeedProps) {
  ...
  const displayed = showAll
    ? (events ?? [])
    : (events ?? []).filter((e) => !LOW_SIGNAL_TYPES.includes(e.event_type))

  const hiddenCount = (events?.length ?? 0) - displayed.length
  const totalCount = events?.length ?? 0

  // Build rows with date separators
  const rows: Array<{ type: 'separator'; label: string } | { type: 'event'; event: ActivityEvent }> = []
      ...
      <CardContent className="pt-0">
        {isLoading ? (
          ...
          <p className="py-4 text-center text-sm text-muted-foreground">
            {totalCount > 0 ? 'No highlights yet — click All to see all events' : 'No activity yet'}
          </p>
        ) : (
          <div>
            {rows.map((row, i) =>
              row.type === 'separator' ? (
                ...
                <div key={row.event.id} className={cn('border-b last:border-b-0')}>
